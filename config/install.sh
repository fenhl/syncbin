...
if which python3 > /dev/null 2>&1; then
    ...
    #if [ $pi_reinstall = yes ]; then
    #    ${HOME}/berryconda3/bin/pip install click docopt python-mpd2 pytz requests tzlocal # when changing this, also change below
    ...
    if which pip3 > /dev/null 2>&1; then
        ...
        if [ "${install_python_packages_using_sudo}" = 'yes' ]; then
            sudo pip3 install click docopt python-mpd2 pytz requests tzlocal # when changing this, also change above and below
        else
            pip3 install click docopt python-mpd2 pytz requests tzlocal # when changing this, also change above
        fi
