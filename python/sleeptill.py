...
def sleep_until(end_date):
    ...
    if end_date <= datetime.utcnow():
        return
    time.sleep((end_date - datetime.utcnow()).total_seconds())
...
if __name__ == '__main__':
    arguments = docopt(__doc__, version='sleeptill from fenhl/syncbin ' + __version__)
    if arguments['<date>']:
        end_date = datetime.strptime(arguments['<date>'] + ' ' + arguments['<time>'], '%Y-%m-%d %H:%M:%S')
    else:
        hours, minutes, seconds = (int(time_unit) for time_unit in arguments['<time>'].split(':'))
        end_date = datetime.combine(date.today(), dtime(hours, minutes, seconds))
